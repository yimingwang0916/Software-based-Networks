...
#        # (1,2), (2,'H2') easy to get port number
#        go_path_pair = zip(path[1:-1], path[2:])
#        back_path_pair = zip(path[::-1][1:-1], path[::-1][2:])
#
#        for s, t in go_path_pair:
##            out_port = G.nodes[node_id]['']
...
#            self.program_flow(self.datapaths[s], match, actions, priority=10, hard_timeout=0, idle_timeout=0)
#        for s, t in back_path_pair:
##            out_port = G.nodes[node_id]['']
#            match = parser.OFPMatch(
#                eth_type = ether_types.ETH_TYPE_IP,
#                ipv4_src = ip.dst, # reverse here
#                ipv4_dst = ip.src
#            )
#            actions = [parser.OFPActionOutput(out_port)]
#            self.program_flow(self.datapaths[s], match, actions, priority=10, hard_timeout=0, idle_timeout=0)
#        # now send this packet back
#        u = path.index(datapath.id) # get current place in the computed path
##        out_port = G[path[u]][path[u+1]]['port']
#        self.send_pkt(datapath, data, port=out_port)
        ...
        #print(ip_src)
        #print(ip_dst)
        #print("host_src", host_src)
        #print("host_dst", host_dst)
        #print("path", new_path)
        path_only_switches = new_path[1:]
        #print("only_switches", path_only_switches)
        #print("*"*20)

        #print("")
        for idx, node_id in enumerate(path_only_switches):
            #print("")
            #print("index=", idx, "node_id=", node_id, G.nodes[node_id])

            if node_id  > 10:
                break;
            next_hop = path_only_switches[idx+1]

            #print("next element in list", next_hop )

            key_for_switch_attribute = 'to_s%d' % next_hop

            #print("key", key_for_switch_attribute)
            ...
            #self.send_pkt(datapath, data, port = output_port)

        #print("swiches", G.nodes(data=True))
        #print("links", G.edges)
        #print("shortest path from H1 to H2", nx.shortest_path(G,source=11,target=12))
        #
        #new_path = nx.shortest_path(G,source=11,target=12)
        #
        #print("path")
        ...
        #  switch_name = G.nodes[node_id]['name']
        #  target = 'to_s2'
        #  output_port =  G.nodes[node_id][target]
        #  ipv4_src = '11.2.4.6' # extracted from packet
        #  ipv4_dst = '22.4.7.6' # extracted from packet
        #  print("install rule at switch %s with src=.. dst=.. and action ouput port %s" % (str(switch_name), str(output_port)))
